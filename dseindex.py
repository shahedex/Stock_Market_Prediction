...
query = "SELECT * from dsedayendsumm"
try:
    cur.execute(query)
...
def current_mcap(date):
    total_mcap = 0
    query = "SELECT * from dsedayendsumm WHERE `DATE`='%s'"
    cur.execute(query %(date))
    results = cur.fetchall()
    mcap = 0
    for result in results:
        mcap += (float(result[3].replace(',','')) * float(result[11].replace(',','')))
    total_mcap += mcap
    return total_mcap
...
def closing_mcap(date):
    ...
    mcap = 0
    for result in results:
        mcap += (float(result[7].replace(',','')) * float(result[11].replace(',','')))
    total_mcap += mcap
    return total_mcap
